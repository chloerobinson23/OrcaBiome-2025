...
names(rare.df)[4:8]<-c("SampleID","Type","WhaleID","Sex","Population")

# Create factors
rare.df$Type <- factor(rare.df$Type, 
                       levels = c("BL","FP"),
...
p1.tmp <- ggplot(data = rare.df) +
  ...
  labs(x="Reads", y="ESVs") +
  geom_point(aes(x = raw.read, y = OTU, color = Type), size=0.1) +
...
ggsave("FigS4_rarefaction.jpeg", plot = g, dpi = 300, height = 6, width = 8, units = "in")
