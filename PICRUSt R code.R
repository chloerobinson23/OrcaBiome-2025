...
my_sequences <- read_csv("my_sequences_2.csv")  # columns: OTU_ID, Sequence
...
# EC predictions
bac_ec <- read_tsv("//wsl.localhost/Ubuntu/home/cetusecologist/picrust_Dec_v2_inputs/picrust2_out/bac_EC_predicted.tsv.gz")

# KO predictions
bac_ko <- read_tsv("//wsl.localhost/Ubuntu/home/cetusecologist/picrust_Dec_v2_inputs/picrust2_out/bac_KO_predicted.tsv.gz")

# Pathways
pathways <- read_tsv("//wsl.localhost/Ubuntu/home/cetusecologist/picrust_Dec_v2_inputs/picrust2_out/pathways_out/path_abun_unstrat.tsv.gz")

# NSTI
bac_nsti <- read_tsv("//wsl.localhost/Ubuntu/home/cetusecologist/picrust_Dec_v2_inputs/picrust2_out/bac_marker_predicted_and_nsti.tsv.gz")
...
#0.00015  0.01522  0.04872  0.22123  0.12284 28.73649 
...
#data:  diffs
#W = 0.93175, p-value = 0.2084

# Paired t-test
t.test(wide$No, wide$Yes, paired = TRUE)

#data:  wide$No and wide$Yes
#t = -10.209, df = 17, p-value = 1.142e-08
...
#t = -10.9194 → the magnitude of difference relative to variability is large
#p = 1.167e-08 < 0.05 → reject the null hypothesis
#There is a significant difference between skin-associated and general pathways.
#Mean difference = -2909250 → on average, skin-associated pathways are over 290k units higher than general pathways.
